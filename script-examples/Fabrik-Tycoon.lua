--[[
  [UPD] Fabrik-Tycoon — remote + touch auto farmer (Rayfield UI)
  Place: 15197136141
  ...
  UI: https://github.com/SiriusSoftwareLtd/Rayfield
]]
...
local function shutdown(notify)
	...
	if G.__FabrikFarmRayfield then
		pcall(function() G.__FabrikFarmRayfield:Destroy() end)
		G.__FabrikFarmRayfield = nil
	end
	destroyLegacyGui()
	setPhase("unloaded")
...
local rayfieldOk, rayfieldErr = runStep("loadRayfield", function()
	Rayfield = loadstring(game:HttpGet("https://sirius.menu/rayfield"))()
end)
if not rayfieldOk or not Rayfield then
	logError("Rayfield failed to load — aborting: " .. tostring(rayfieldErr))
	shutdown(false)
	return
end
G.__FabrikFarmRayfield = Rayfield
...
local function getFlag(name, fallback)
	...
	local f = flags and flags[name]
	if f and f.CurrentValue ~= nil then
...
local function syncConfigFromFlags()
	Config.Enabled = getFlag("MasterFarm", Config.Enabled)
	Config.AutoCollect = getFlag("AutoCollect", Config.AutoCollect)
	Config.AutoButtons = getFlag("AutoButtons", Config.AutoButtons)
	Config.AutoGemUpgrades = getFlag("AutoGemUpgrades", Config.AutoGemUpgrades)
	Config.AutoRebirth = getFlag("AutoRebirth", Config.AutoRebirth)
	Config.AutoManualDropper = getFlag("AutoManualDropper", Config.AutoManualDropper)
	Config.HideMonetization = getFlag("HideMonetization", Config.HideMonetization)
	Config.LoopDelay = getFlag("LoopDelay", Config.LoopDelay)
	Config.CollectMin = math.floor(getFlag("CollectMin", Config.CollectMin) + 0.5)
	Config.RebirthInterval = getFlag("RebirthInterval", Config.RebirthInterval)
	Config.VerboseLogging = getFlag("VerboseLogging", Config.VerboseLogging)
...
-- ===================== Rayfield UI =====================
-- Built in a nested function: main chunk was over Luau's 200 local register limit.
local function buildRayfieldUI()
	...
	local Window = Rayfield:CreateWindow({
		Name = "Fabrik-Tycoon Farm",
		Icon = 0,
		LoadingTitle = "Fabrik-Tycoon",
		LoadingSubtitle = "Auto farm by Dexy",
		Theme = "Amber",
		DisableRayfieldPrompts = false,
		DisableBuildWarnings = false,
		ConfigurationSaving = {
			Enabled = true,
			FolderName = "FabrikTycoon",
			FileName = "FabrikFarmConfig",
		},
	})
	...
	-- ---------- Farm tab: the only things you touch day to day ----------
	...
	FarmTab:CreateToggle({
		Name = "Buy buildings",
		CurrentValue = Config.AutoButtons,
		Flag = "AutoButtons",
		Callback = function(v) Config.AutoButtons = v end,
	})

	FarmTab:CreateToggle({
		Name = "Gem upgrades",
		CurrentValue = Config.AutoGemUpgrades,
		Flag = "AutoGemUpgrades",
		Callback = function(v) Config.AutoGemUpgrades = v end,
	})
	...
	FarmTab:CreateToggle({
		Name = "Manual droppers",
		CurrentValue = Config.AutoManualDropper,
		Flag = "AutoManualDropper",
		Callback = function(v) Config.AutoManualDropper = v end,
	})

	FarmTab:CreateSection("Live status")
	statusLabel = FarmTab:CreateLabel(statusLine())

	-- ---------- Progress tab: what's built and what the farm is chasing ----------
	local ProgressTab = Window:CreateTab("Progress", 4483362458)
	ProgressTab:CreateSection("Live progression")
	progressLabel = ProgressTab:CreateParagraph({
		Title = "Progression",
		Content = progressionContent(),
	})
	ProgressTab:CreateButton({
		Name = "Refresh now",
		Callback = function() updateProgressLabel(true) end,
	})

	-- ---------- Settings tab: tuning, extras, tools ----------
	SettingsTab:CreateSection("Tuning")
	...
	SettingsTab:CreateSlider({
		...
		Callback = function(v) Config.RebirthInterval = v end,
	})

	SettingsTab:CreateSection("Extras")

	SettingsTab:CreateToggle({
		Name = "Hide Robux / gamepass ads",
		CurrentValue = Config.HideMonetization,
		Flag = "HideMonetization",
		Callback = function(v) setAdHidingEnabled(v) end,
	})
	...
	SettingsTab:CreateSection("Danger")

	SettingsTab:CreateButton({
		Name = "Quit — stop farm & unload",
...
runStep("loadConfiguration", function() Rayfield:LoadConfiguration() end)
task.wait(0.35)

-- Guarantee all toggles start OFF on every load, even if a saved config had
-- them ON. Sliders keep their saved values; only the toggle flags are reset.
runStep("forceTogglesOff", function()
	local flags = Rayfield and Rayfield.Flags
	for _, name in {
		"MasterFarm", "AutoCollect", "AutoButtons", "AutoGemUpgrades",
		"AutoRebirth", "AutoManualDropper", "HideMonetization", "VerboseLogging",
	} do
		local f = flags and flags[name]
		if f and f.Set then
			pcall(function() f:Set(false) end)
		end
	end
end)
...
setPhase("ready")
logInfo("Loaded (Rayfield) — all toggles default OFF | " .. statusLine())
