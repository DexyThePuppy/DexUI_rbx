--[[ Fabrik-Tycoon — all game logic. Hub: scripts/fabrik-tycoon.lua ]]
...
return function(DexUI)
	...
	-- init
		...
		if not ctx.events then
			ctx.log.error("Events folder missing â€” wrong game? aborting")
			ctx.shutdown(false)
			return
		end

		if not ctx.other then
			ctx.log.error("Fabrik API helper did not load Scripts.Other â€” aborting")
			ctx.shutdown(false)
			return
		end
	...
	local function builtSummary()
		...
		return string.format("Built %d / %d pads  Ãƒâ€šÃ‚Â·  %d income pads built", bought, total, incomeBuilt)
	end
	...
	local function statusLine()
		...
		return string.format(
			"%s %s Ãƒâ€šÃ‚Â· pool %s (df %s) Ãƒâ€šÃ‚Â· gems %d Ãƒâ€šÃ‚Â· R%d Ãƒâ€šÃ‚Â· reb %d/%d",
	...
	local function statsLine()
		...
		if msg and msg ~= "" then
			return string.format("Cycles %d Ãƒâ€šÃ‚Â· Errors %d Ãƒâ€šÃ‚Â· %s", stats.cycles, stats.errors, msg)
		end
		return string.format("Cycles %d Ãƒâ€šÃ‚Â· Errors %d Ãƒâ€šÃ‚Â· phase %s", stats.cycles, stats.errors, ctx.log.phase)
	end
