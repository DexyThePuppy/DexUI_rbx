--[[
  [UPD] Fabrik-Tycoon — DexUI auto farmer (single hub script)
  Place: 15197136141

  scripts/sdk/           DexUI session, loops, notifications
  scripts/fabrik.lua     Bare Fabrik helpers (tycoon API, formatting)
  scripts/fabrik-tycoon.lua   This file — all game-specific logic
]]
...
local function initGame(ctx)
	...
	ctx.runStep("requireOther", function()
		if not loadFile("fabrik.lua")(ctx) then
...
-- What the farm is doing this cycle (for Progress tab â€” matches tryBuyCheapestButton).
...
tryBuyCheapestButton = function()
	...
	-- Only cosmetics remain â€” buy the cheapest affordable one so progression
...
local function builtSummary()
	...
	return string.format("Built %d / %d pads  Â·  %d income pads built", bought, total, incomeBuilt)
end
...
progressionContent = function()
	...
	if intent.mode == "none" then
		lines[#lines + 1] = "No buyable pads right now â€” rebirth or a gated"
		lines[#lines + 1] = "unlock may be blocking the next area."
		return table.concat(lines, "\n")
	end
	if intent.mode == "cosmetics" then
		lines[#lines + 1] = "Only decor pads left â€” buying those to unblock chains."
		return table.concat(lines, "\n")
	end
	...
	elseif intent.mode == "hoard" then
		...
		lines[#lines + 1] = "  Close to goal â€” not buying cheaper pads anymore."
	else
		lines[#lines + 1] = "End goal:  " .. target.name
		lines[#lines + 1] = string.format(
			"  %s of %s  (%d%% toward goal)",
		...
		if intent.nextPad and intent.nextPad.name ~= target.name then
			...
			if intent.pool and intent.pool > 0 then
				...
				if combined >= intent.nextPad.price then
					lines[#lines + 1] = string.format(
						"  Pool %s â€” collect to afford next pad",
...
local function statusLine()
	...
	return string.format(
		"%s %s Â· pool %s (df %s) Â· gems %d Â· R%d Â· reb %d/%d",
...
local function statsLine()
	...
	if msg and msg ~= "" then
		return string.format("Cycles %d Â· Errors %d Â· %s", stats.cycles, stats.errors, msg)
	end
	return string.format("Cycles %d Â· Errors %d Â· phase %s", stats.cycles, stats.errors, ctx.log.phase)
end
...
local SDK = loadFile("sdk/run.lua")
